...
import { Heart, Star, SlidersHorizontal, Award } from 'lucide-react';
...
const fallbackProducts = [
  { id: 1, name: 'Bestseller: Pink Silk Saree', image: product1, price: 12999, originalPrice: 18999, rating: 4.5, reviews: 128, bestseller: true },
  { id: 2, name: 'Fan Favorite: Maroon Lehenga', image: product2, price: 15999, originalPrice: 22999, rating: 4.8, reviews: 256, bestseller: true },
  { id: 3, name: 'Top Rated: Teal Saree', image: product3, price: 9999, originalPrice: 14999, rating: 4.3, reviews: 89, bestseller: true },
  { id: 4, name: 'Customer Fave: Yellow Lehenga', image: product4, price: 24999, originalPrice: 35999, rating: 4.9, reviews: 312, bestseller: true },
  { id: 5, name: 'Most Loved: Red Bridal Set', image: product5, price: 45999, originalPrice: 65999, rating: 5.0, reviews: 456, bestseller: true },
  { id: 6, name: 'Top Pick: Green Silk Saree', image: product6, price: 11999, originalPrice: 16999, rating: 4.4, reviews: 167, bestseller: true },
  { id: 7, name: 'Favorite: Pink Anarkali', image: product7, price: 18999, originalPrice: 27999, rating: 4.6, reviews: 198, bestseller: true },
  { id: 8, name: 'Classic: Black Tissue Saree', image: product8, price: 13999, originalPrice: 19999, rating: 4.7, reviews: 234, bestseller: true },
];
...
const CultFaves = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const params = { isBestseller: 'true', limit: 50 };
        if (sortBy === 'price-low') {
          params.sortBy = 'price';
          params.order = 'asc';
  ...
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <div className="relative h-[300px] md:h-[400px] bg-gradient-to-r from-blue-200/40 to-purple-200/40">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center px-4">
            <div className="flex items-center justify-center gap-3 mb-4">
              <Award className="text-primary" size={32} />
              <h1 className="font-display text-4xl md:text-6xl text-foreground">Cult Faves</h1>
            </div>
            <p className="font-body text-lg text-muted-foreground max-w-2xl mx-auto">
              Customer-loved bestsellers that never go out of style
            </p>
          </div>
        </div>
      </div>
            ...
            {products.map((product, index) => (
              <Link key={product._id || product.id} to={`/product/${product._id || product.id}`} className="group product-card animate-fade-up" style={{ animationDelay: `${index * 0.05}s` }}>
                <div className="relative overflow-hidden aspect-[3/4]">
                  ...
                  {product.bestseller && (
                    <div className="absolute top-3 left-3 bg-blue-600 text-white px-2 py-1 text-xs font-body font-semibold rounded-sm flex items-center gap-1">
